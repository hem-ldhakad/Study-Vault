...
export const NoteCard = ({ note, onBookmarkToggle, isBookmarked = false }) => {
  ...
  return (
    <motion.div
      ...
      className="glass-panel bg-white rounded-3xl overflow-hidden flex flex-col justify-between shadow-md border border-slate-200 hover:border-indigo-500 transition-all duration-300 group"
    >
      <div>
        {/* Card Thumbnail Container */}
        <div className="relative h-48 bg-gradient-to-br from-indigo-100 via-purple-50 to-pink-100 flex items-center justify-center overflow-hidden border-b border-slate-200">
          {thumbnailSrc ? (
            <img
              src={thumbnailSrc}
              alt={note.title}
          ...
          {onBookmarkToggle && (
            <button
              ...
              className={`absolute top-3 right-3 p-2 rounded-2xl backdrop-blur-md transition-all shadow-md ${
                isBookmarked
